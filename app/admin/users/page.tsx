...
import { WorkspaceAccessDialog } from "@/components/admin/workspace-access-dialog"
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function UsersPage() {
  ...
  const handleWorkspaceAccess = (user: User) => {
    setTimeout(() => {
      setSelectedUser(user)
      setIsWorkspaceAccessOpen(true)
    }, 0)
  }

  const handleCloseWorkspaceAccess = () => {
    setIsWorkspaceAccessOpen(false)
    setTimeout(() => {
      setSelectedUser(null)
    }, 100)
  }
  ...
  return (
    <div className="space-y-6 ">
      <div>
        <h1 className="text-[22px] font-semibold text-zinc-900 tracking-tight">User management</h1>
        <p className="text-sm text-zinc-600">Manage your team members and their account permissions here.</p>
      </div>
        ...
        <Table>
          <TableHeader>
            <TableRow className="hover:bg-transparent">
              ...
              <TableHead className="py-3 text-[13px] text-zinc-500 font-medium bg-zinc-50">Workspace access</TableHead>
            ...
            {isLoading ? (
              ...
              currentUsers.map((user) => (
                <TableRow key={user.id} className="border-t border-zinc-200 hover:bg-zinc-50/50">
                  ...
                  <TableCell className="py-4">
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center text-[15px] font-medium">
                  ...
                  <TableCell className="py-4">
                    <div className="flex items-center gap-2">
                      <div className="rounded-full px-2.5 py-1 text-[13px] font-medium bg-emerald-50 text-emerald-700">
                        {user.role === "admin" || user.role === "org_admin" ? "Admin" : "User"}
                      </div>
                    </div>
                  </TableCell>
                  <TableCell className="py-4">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => handleWorkspaceAccess(user)}
                      className="h-8 px-3 text-[13px] font-medium border-zinc-200 text-zinc-700 hover:bg-zinc-50 hover:text-zinc-700"
                    >
                      Manage access
                    </Button>
                  </TableCell>
      ...
      {isWorkspaceAccessOpen && selectedUser && (
        <WorkspaceAccessDialog open={isWorkspaceAccessOpen} onClose={handleCloseWorkspaceAccess} user={selectedUser} />
      )}
    </div>
  )
}
