...
export function EditUserDialog({ open, onClose, user, onUserUpdated }: EditUserDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to update super admin user")
      }
      ...
      toast.error("Failed to update super admin user")
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <DialogContent className="sm:max-w-[525px] bg-white border border-zinc-300 shadow-xl">
        <DialogHeader>
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <DialogTitle className="text-xl font-semibold text-zinc-900">Edit Super Admin</DialogTitle>
              <DialogDescription className="text-[15px] text-zinc-500">
                Update super admin user account information.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
